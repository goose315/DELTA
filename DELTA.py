...
def weighted_kmeans(X, weights, n_clusters, max_iter=300, tol=1e-4):
    ...
    # Step (a): Initialize centroids using KMeans++
    kmeans = KMeans(n_clusters=n_clusters, init='k-means++', random_state=0)
    kmeans.fit(X)
    centroids = kmeans.cluster_centers_
    
    for iteration in range(max_iter):
        # Step (b): Assign clusters with weighted distance
        ...
        for k in range(n_clusters):
            ...
            if len(cluster_indices) > 0:
                ...
                new_centroids[k] = np.sum(X[cluster_indices] * cluster_weights[:, np.newaxis], axis=0) / np.sum(cluster_weights)
...
all_scores_weighted_page = weightedkhop_entropy_scores_cpu + normalized_similarities_embedding
...
different_logits_branch1_np = different_logits_branch1.detach().cpu().numpy()
different_encoded_output_branch1_np = different_encoded_output_branch1.detach().cpu().numpy()

weights = all_scores_weighted_page[different_indices_weighted_page].flatten()

kmeans = KMeans(n_clusters=args.n_clusters, random_state=args.seed)

kmeans.fit(different_logits_branch1_np, sample_weight=weights)
labels = kmeans.labels_
centroids = kmeans.cluster_centers_


#print("Weighted K-Means cluster labels:", labels)

# Find the maximum value of all_scores for each cluster and the corresponding original index
unique_labels = np.unique(labels)
max_scores_per_cluster = []
max_indices_per_cluster = []

for label in unique_labels:
    cluster_indices = np.where(labels == label)[0]
    selected_indices = different_indices_weighted_page[cluster_indices].tolist()
    cluster_scores = all_scores_weighted_page[selected_indices]
    max_index = cluster_scores.argmax()
    max_score = cluster_scores[max_index]
    max_scores_per_cluster.append(max_score)
    max_indices_per_cluster.append(selected_indices[max_index])

# Assign pseudo-labels and update masks
label_rate_target = 0.0
target_train_size = int(target_data.size(0) * label_rate_target)
label_mask_weightedkhop_entropy_page = np.array([1] * target_train_size + [0] * (target_data.size(0) - target_train_size)).astype(bool)
label_mask_weightedkhop_entropy_page[max_indices_per_cluster] = True
label_mask_weightedkhop_entropy_page = torch.tensor(label_mask_weightedkhop_entropy_page).to(device)

unique, counts = torch.unique(target_data.y[max_indices_per_cluster], return_counts=True)
count_dict = dict(zip(unique.cpu().numpy(), counts.cpu().numpy()))
print("DELTA:", count_dict)
...
def train_DELTA(epoch):
    ...
    cls_loss2 = CrossEntropyLoss(target_logits[label_mask_weightedkhop_entropy_page], target_data.y[label_mask_weightedkhop_entropy_page])
    ...
    target_pseudo_label = target_softmax_out.argmax(dim=1)
    source_pseudo_label[label_mask] = source_data.y[label_mask]
    target_pseudo_label[label_mask_weightedkhop_entropy_page] = target_data.y[label_mask_weightedkhop_entropy_page]
...
for epoch in range(1, epochs):
    ...
    source_encoded_output_DELTA, source_logits_DELTA, source_preds_DELTA, source_correct, _, _ = test_DELTA(source_data, 'source', source_data.test_mask)
    target_encoded_output_DELTA, target_logits_DELTA, target_preds_DELTA, target_correct, macro_f1, micro_f1 = test_DELTA(target_data, 'target')
    #print("Epoch: {}, source_acc: {}, target_acc: {}, macro_f1: {}, micro_f1: {}".format(epoch, source_correct, target_correct, macro_f1, micro_f1))
    if source_correct > best_source_acc:
        best_target_acc = target_correct
        best_source_acc = source_correct
