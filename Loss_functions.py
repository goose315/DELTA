...
class CustomSupConLoss(SupConLoss):
    def forward(self, features, labels=None, mask=None):
        ...
        batch_size = features.shape[0]
        mask = mask or torch.eye(batch_size, dtype=torch.float32).to(device)
        ...
        mask_pos_pairs = mask.sum(1)
        mask_pos_pairs = torch.where(mask_pos_pairs < 1e-6, torch.tensor(1).to(device), mask_pos_pairs.long())
        mean_log_prob_pos = (mask * log_prob).sum(1) / mask_pos_pairs.float()
